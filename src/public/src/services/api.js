...
const getAxios = () => (typeof axios !== "undefined" ? axios : window.axios);
...
export async function loginUser(email, password) {
    try {
        const response = await getAxios().post(`${API_BASE_URL}/login`, {
            email: email.trim(),
...
export async function registerUser({ name, age, department, email, password }) {
    try {
        const response = await getAxios().post(`${API_BASE_URL}/register`, {
            name: name.trim(),
            age: Number(age),
...
export async function addStudent(name, age, department) {
    try {
        const response = await getAxios().post(`${API_BASE_URL}/students`, {
            name: name.trim(),
            age: Number(age),
            department: department.trim()
        });
...
export async function fetchUserUploads(userEmail) {
    try {
        const response = await getAxios().get(`${API_BASE_URL}/upload/files`, {
...
export async function uploadSingleFile(file, uploadedByEmail) {
    ...
    try {
        const response = await getAxios().post(`${API_BASE_URL}/upload`, formData, {
